...
void BEEP_Task(void)
{
	...
	if (iAutoTuneCnt > 0 )
	{
		...
		ssDACValueR = (int16_t)((usBeepWave[(iBeepTimeCnt/1024) & 0x000003FF] * beepVol) / 16384/2);
	...
	else
	{
		...
		// Apply the beep only to the ear phone
		ssDACValueR = (int16_t)((usBeepWave[(iBeepTimeCnt/1024) & 0x000003FF] * iADSRCnt) / 16384/4);
	}
