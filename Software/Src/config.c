...
CONFIG_eConfigEntry CONFIG_NameToEnum(char* name)
{
	...
	if (strcmp(name, "ADDSYNTH_1") == 0) return CFG_E_ADDSYNTH_1;
	...
	if (strcmp(name, "RICHNESS") == 0) return CFG_E_RICHNESS;
...
char* CONFIG_EnumToName(CONFIG_eConfigEntry enumName)
{
	...
	if (enumName == CFG_E_ADDSYNTH_1) return "ADDSYNTH_1";
	...
	if (enumName == CFG_E_RICHNESS) return "RICHNESS";
